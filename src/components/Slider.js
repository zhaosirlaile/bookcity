...
import {Dimensions, View, Image, StyleSheet, PanResponder} from 'react-native';
...
export default class MySlider extends React.Component {
  static propTypes = {
    ...
    myOnChange: PropTypes.func, // 进度值发生改变时的回调
    onAfterChange: PropTypes.func, // 拖动结束时的回调
    defaultValue: PropTypes.number, // 默认的进度值
    minNumber: PropTypes.number.isRequired, // 进度范围最小值
    maxNumber: PropTypes.number.isRequired, // 进度范围最大值
  ...
  componentWillMount() {
    ...
    const {defaultValue, minNumber, maxNumber, thumbSize} = this.props;
    const process = defaultValue / (maxNumber - minNumber);
  ...
  _changeProcess(changeProcess) {
    ...
    const {minNumber, maxNumber, step, myOnChange} = this.props;
    const {process} = this.state;

    if (changeProcess >= 0 && changeProcess <= 1) {
      myOnChange(changeProcess);

      // 按步长比例变化刻度
      const v = changeProcess * (maxNumber - minNumber);
      const newValue = Math.round(v / step) * step;
      const newProcess = newValue / (maxNumber - minNumber);
